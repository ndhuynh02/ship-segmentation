...
from src.utils.airbus_utils import get_boxes, rotate_nms, midpoint2corners, mask_overlay, download_checkpoint
...
def main(args):
    ...
    try:
        ...
        print("Error in checkpoint path. Using default checkpoint")
        download_checkpoint()
        model = YoloXLitModule.load_from_checkpoint(
            'ckpt/yoloX.ckpt', net=YoloX()
        )
    ...
    overlay_image = mask_overlay(image, instance_mask)
    cv2.imwrite(args.output, overlay_image[:, :, ::-1])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Ship instance segmentation inferrence')
    parser.add_argument('--image', help='path to image file')
    parser.add_argument('--output', help='path to output image (default: instance.png)', default='instance.png')
    parser.add_argument('--device', default='cpu', help='inference device')
    parser.add_argument('-ckpt', help='path to checkpoint (.ckpt file)', default='checkpoints/yoloX.ckpt')
