...
class AirbusDataModuleClassifier(LightningDataModule):
    ...
    def setup(self, visualize_dist=False, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            dataframe["counts"] = dataframe.apply(
                lambda row: 0 if pd.isnull(row["EncodedPixels"]) else 1, axis=1
            )
            unique_img_ids = dataframe.groupby("ImageId")["counts"].sum().reset_index()

            # Try catch block for stratified splits
            try:
                # split ship image first -> make sure the train, val, test images are the same with the segmentation model
                ship_ids = unique_img_ids[unique_img_ids["counts"] != 0]

                train_ids, valid_and_test_ids = train_test_split(
                    ship_ids,
                    train_size=self.hparams.train_val_test_split[0],
                    stratify=ship_ids["counts"],
                    shuffle=True,
                    random_state=42,
                )
                ...
                assert len(train_ids) + len(val_ids) + len(test_ids) == len(ship_ids)

                if visualize_dist:
                    self.visualize_dist(dataframe, train_ids, val_ids, test_ids)
            ...
            # split non-ship images with the same ratio
            non_ship_ids = unique_img_ids[unique_img_ids["counts"] == 0]

            non_ship_train, non_ship_val_and_test = train_test_split(
                non_ship_ids,
                train_size=self.hparams.train_val_test_split[0],
                shuffle=True,
                random_state=42,
            ...
            non_ship_val, non_ship_test = train_test_split(
                non_ship_val_and_test,
                train_size=self.hparams.train_val_test_split[1]
                / (self.hparams.train_val_test_split[1] + self.hparams.train_val_test_split[2]),
                shuffle=True,
                random_state=42,
            ...
            non_ship_train_subset = Subset(dataset, non_ship_train.index.to_list())
            non_ship_val_subset = Subset(dataset, non_ship_val.index.to_list())
            non_ship_test_subset = Subset(dataset, non_ship_test.index.to_list())

            # combine ship & non-ship images
            self.data_train += non_ship_train_subset
            self.data_val += non_ship_val_subset
            self.data_test += non_ship_test_subset
            ...
            assert dataframe["ImageId"].nunique() == len(self.data_train) + len(
                self.data_val
            ) + len(self.data_test)
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        loader = airbus.train_dataloader()
