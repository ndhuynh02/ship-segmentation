...
class ResMUnetLitModule(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
    ...
    def model_step(self, batch: Any):
        ...
        outputs_m = self.forward(input)

        if isinstance(self.criterion, (LossBinary, BCE_Lovasz, FocalBCELoss)):
            cnt1 = (labels_m == 1).sum().item()  # count number of class 1 in image
            cnt0 = labels_m.numel() - cnt1
            if cnt1 != 0:
        ...
        return loss, outputs_m, labels_m, file_id

    def training_step(self, batch: Any, batch_idx: int):
        loss, outputs_m, labels_m, _ = self.model_step(batch)

        # update and log metrics
        self.train_loss(loss)
        self.train_metric(outputs_m, labels_m)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        loss, outputs_m, labels_m, file_id = self.model_step(batch)

        # update and log metrics
        self.val_loss(loss)
        self.val_metric(outputs_m, labels_m)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        # update and log metrics
        self.test_loss(loss)
        self.test_metric(outputs_m, labels_m)
