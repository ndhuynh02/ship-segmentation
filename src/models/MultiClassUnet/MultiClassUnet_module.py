...
class MultiClassUnetLitModule(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
        ...
        self.train_metric = JaccardIndex(task="multiclass", num_classes=3)
        self.val_metric = JaccardIndex(task="multiclass", num_classes=3)
        self.test_metric = JaccardIndex(task="multiclass", num_classes=3)
    ...
    def model_step(self, batch: Any):
        ...
        labels_new_c = sample["label_new_c"].to(self.device)
        file_id = sample["file_id"]
        weights = None
        if "weight" in sample:
            weights = sample["weight"].to(self.device)

        outputs_new_c = self.forward(input)

        if isinstance(self.criterion, (LossBinary, BCE_Lovasz, FocalBCELoss)):
            cnt1 = (labels_new_c == 1).sum().item()  # count number of class 1 in image
            cnt0 = labels_new_c.numel() - cnt1
            if cnt1 != 0:
        ...
        loss = self.criterion(outputs_new_c, labels_new_c)

        # Code to try to fix CUDA out of memory issues
        gc.collect()
        torch.cuda.empty_cache()

        return loss, outputs_new_c, labels_new_c, file_id

    def training_step(self, batch: Any, batch_idx: int):
        loss, outputs_new_c, labels_new_c, _ = self.model_step(batch)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        loss, outputs_new_c, labels_new_c, file_id = self.model_step(batch)
        ...
        return {
            "loss": loss,
            "labels_new_c": labels_new_c,
            "outputs_new_c": outputs_new_c,
            "file_id": file_id,
        }
        # return {"loss": loss}
