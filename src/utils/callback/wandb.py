...
class WandbCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        #log ra 25 ảnh xấu, mỗi loại ảnh xấu 1 key
        list_error_real = []
        list_error_pred = []
        i = 1
        ...
        for i in range(5):
            stack_bad_pred = torch.stack(list_error_pred_type_i[i])
            stack_bad_real = torch.stack(list_error_real_type_i[i])
            grid_bad_pred = make_grid(stack_bad_pred, nrow=len(list_error_pred_type_i[i]))
            grid_bad_real = make_grid(stack_bad_real, nrow=len(list_error_real_type_i[i]))
    ...
    def on_validation_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        wandb_logger = trainer.logger
        wandb_logger.log_image(key='eval images', images=[grid_pred_np, grid_target_np])
