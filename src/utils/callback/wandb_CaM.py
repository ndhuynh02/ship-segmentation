...
from src.models.CaMUnet.components.helper import partition_instances
...
class Callback(Callback):
    ...
    def on_validation_batch_end(
        ...
        preds = np.squeeze(torch.sigmoid(outputs["outputs"]).cpu().numpy(), axis=1)
        preds_c = np.squeeze(torch.sigmoid(outputs["outputs_c"]).cpu().numpy(), axis=1)
