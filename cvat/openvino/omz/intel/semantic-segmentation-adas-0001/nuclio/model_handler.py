...
class ModelHandler:
    ...
    def infer(self, image, threshold):
        ...
        for i in range(len(self.labels)):
            ...
            mask_by_label = cv2.resize(
                mask_by_label, dsize=(image.width, image.height), interpolation=cv2.INTER_NEAREST
            )

            contours, _ = cv2.findContours(
                mask_by_label, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
