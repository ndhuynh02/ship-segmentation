...
class ModelHandler:
    ...
    def handle(self, image, points):
        ...
        points = (
            points * [DEXTR_SIZE / numpy_cropped.shape[1], DEXTR_SIZE / numpy_cropped.shape[0]]
        ).astype(int)
        ...
        for point in points:
            ...
            gaussian = np.exp(
                -4 * np.log(2) * ((gaussian_x_axis**2 + gaussian_y_axis**2) / 100)
            ).astype(np.float64)
        ...
        pred = cv2.resize(
            pred, tuple(reversed(numpy_cropped.shape[:2])), interpolation=cv2.INTER_NEAREST
        )
